function [lik,derv] = loglike_bandit_forced(CSchosen,Rewards,ntrials,is_instrumental,eta,beta,kappa)
...
Nsub  = 1; % length(TrialType(:,1)); % number of subjects to fit
...
%    Sess = [1,71,141,ntrials];      % the first trial of each session
%else
    Sess = [1,ntrials];             % treat all as one session
%end

numchoices=max(CSchosen);   % ASSUMPTION: choices are labeled 1..numchoices
for subj = 1:Nsub               % we may be fitting more than one subject
    V = zeros(1, numchoices);                  % start with initial values
    dVdEta   = zeros(1,numchoices);      % partial derivatives
    dVdKappa = zeros(1,numchoices);
    
    for s = 1:Nsess             % s = session number
        for t = Sess(s):Sess(s+1)-1  % t = trial number
            
            % the chosen stimulus (1 or 2)
            c = CSchosen(subj,t);
            
            if ~isnan(c)             % this was a valid trial (missed trials: c = NaN)
                
                % the other (nonchosen) stimulus
                n=1:numchoices;
                n=n(n~=c);
                ...
                if(is_instrumental(t))
                    liks(s,subj) = liks(s,subj) + beta(s)*V(c) - log(sum(exp(beta(s).*V)));
                end
                ...
                if (nargout == 2) % we were called by the optimizer and asked for derivatives as well
                    ...
                    exps =  exp(beta(s).*V);
                    p = exp(beta(s)*V(c))/sum(exps);
                    dLdBeta(s)     = dLdBeta(s) + V(c) - ( V*exps' / sum(exps) );
                    dLdEta         = dLdEta + beta(s) * (dVdEta(c) - (dVdEta * exps' / sum(exps) ));
                    %dLdKappa       = dLdKappa + beta(s)*(1-p)*(dVdKappa(c)-sum(dVdKappa(n)));
                end
...
% putting a prior on Beta and Eta (so we are looking for the MAP and not
% the ML solution)
